...
trainSet = dataSet.iloc[rng[2:]]
testSet = dataSet.iloc[rng[0:2]]
...
def forest_gen(tree_num):
    forest = []
    for i in range(tree_num):
        random_samples = bootstrap(trainSet, 25)
        used_features = []
        tree_root = Node([-1, 0, 0])
        tree = Node()
        tree_gen(random_samples, used_features, tree, tree_root)
        forest.append(tree)
        # tr.print_diff()
        print(i)
...
def tree_gen(random_samples, used_features, node, father_node):
    ...
    while (father_node.data[0] != node.data[0] and (
        ...
        if (len(left_child[:, 0]) > 1):
            ...
            tree_gen(left_child, used_features, lnode, node)

        if (len(right_child[:, 0]) > 1):
            rnode = Node()
            node.rchild = rnode
            right_child = pd.DataFrame(right_child)
            tree_gen(right_child, used_features, rnode, node)
...
def count(data):
    ...
    for one in data:
        if one in count_frq:
            ...
            count_frq[one] = 1
    count_frq_v = sorted(count_frq.items(), key=lambda d: d[1])
    count_frq_k = sorted(count_frq.items(), key=lambda d: d[0])
...
def semantic_neighbor(forest, test):
    ...
    for i in range(len(forest)):
        ...
        sn.sort()
        sn_set.extend(sn)
    snn_map = count(sn_set)
    return snn_map


def predict(model):
    for test in np.array(testSet):
        sn=semantic_neighbor(model,test)
        tag = []
        ...
        # T format
        for k,v in sn.items():
            tag.append(label_array[np.argwhere(label_array[:,0]==k)][0][0])
        tag = np.array(tag)
        tag_indexes = tag[:,0]
