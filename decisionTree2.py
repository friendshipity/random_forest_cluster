...
def forest_gen(tree_num, bootstrap_num, feature_per_tree):
    ...
    for i in range(tree_num):
        ...
        for x in np.array(trainSet):  # trainSet or random_samples
            tree_search(tree, x)
        # oob test
        # OOB_error(oob,tree)
        forest.append(tree)
...
def accumulate_order(single_tag_phi):
...
def phi_order(phi):
    label_size = np.shape(phi)[0]
    tree_size = np.shape(phi)[1]
   # score = np.zeros(label_size)
    score = np.zeros(8)
    accumulate = np.zeros(np.shape(phi)[0])
    for i in range(1,tree_size-1):
        accumulate+=phi[i]
        reshape = accumulate.reshape(8, 5)
        _p_sorted = np.argsort(reshape, axis=1)
        mid_winner = _p_sorted[:, 4] + 1
        score=np.vstack((score,mid_winner))
    score = score[1:]
    print("123")
    return score
def quality_filter_mask(_t):
    _p = np.copy(_t)
    for i in range(0,np.shape(_p)[0]):
        sum = np.sum(_p[i])
        _p[i] = _p[i]/sum
    _q = np.copy(_p)
    for i in range(0, np.shape(_q)[0]):
        max = np.max(_q[i])
        _q[i] = _q[i] / max

    _q[_q<0.6]=0
    _o = np.copy(_q)
    for i in range(0, np.shape(_o)[0]):
        if(np.count_nonzero(_o[i])>1):
            _o[i]=np.zeros(np.shape(_o)[1])

    return _o

def predict(model, K_rate):
    K = 0
    true_err_set = []
    mask_nums = []
    for test in np.array(testSet):
        ...
        T_sl = []
        for row in tag_s:
            Ts_row = np.zeros(8 * 5 + 1)
            for indexc, col in enumerate(row[1:]):
                Ts_row[int(indexc * 5 + col)] = 1
            Ts_row[0] = row[0]
            T_sl.append(Ts_row)
        T_s = np.array(T_sl)
        T_s = T_s[:K]
        ...
        # D_d = np.array(D_d)
        D_t = pd.DataFrame(D)
        phi = np.dot(T_s[:, 1:].T, D)
        phi = pd.DataFrame(phi)
        mid_predict_set_basic = phi.T.apply(lambda x: x.sum())
        #
        # weight_init = np.linspace(phi.shape[1], 1, phi.shape[1]) / phi.shape[1]
        # mid_predict_set_weight_plus = phi.T.apply(lambda x: (np.dot(x, weight_init)).sum())
        #
        # h=10
        # mid_predict_set_weight_nearest_h= phi.T.apply(lambda x: np.dot(x[0:h],weight_init[0:h]).sum())


        # mid_predict_set_weight_appear_times = phi.T.apply(lambda x: np.count_nonzero(x))
        # mid_predict_set_order_score


        # phi_order_score = phi_order(phi)


        # mid_predict_set = mid_predict_set_basic



        _p = np.array(mid_predict_set_basic).reshape(8, 5)
        # _p_t = np.array(mid_predict_set_weight_appear_times).reshape(8, 5)
        # _p_w = np.array(mid_predict_set_weight_plus).reshape(8, 5)


        mask = quality_filter_mask(_p)
        mask_num = np.count_nonzero(mask)
        _p=mask*_p
        index_mask = np.ones(8)
        for i in range(0,8):
            if(np.sum(mask[i])==0):
                index_mask[i]=0

        _p_sorted = np.argsort(_p, axis=1)


        mid_predict = _p_sorted[:, 4] + 1
        mid_predict = mid_predict*index_mask
        chosen = np.hstack((1, mid_predict))
        ...
        # chosen = label_array[np.argwhere(label_array[:, 0] == count_frq_v[0][0])][0][0]
        test_label = label_array[np.argwhere(label_array[:, 0] == test[0])][0][0]
        test_label[1:] = test_label[1:]*index_mask
        error_tab = []
        expect_tab = []
        for row in label_array:
            err = error_sum(row[1:], test_label[1:])
...
if __name__ == '__main__':
    ...
    times = 20
    treeNum = 50  # 200   50
    #   bootstrapNum = 225 #small than sample size
    bootstrapNum = 225  # small than sample size
    testNum = 3  # ramdom test sample size
    feature_per_tree = 10     # 108    50
    K_rate = 0.8  # K nearest
    for i in range(times):
        ...
        # train_many(model)
        p,mask_num = predict(model, K_rate)
        print("err" + str(p) +"--lable "+str(mask_num))
        s += p
        label_nums+=mask_num

    print("All " + str(s))
    # print("mean " + str(s / times))
    print("mean err" + str(s / times))
    print("mean times" + str(label_nums / times))
    print("mean acc" + str((label_nums-s)/label_nums ))
